...
| Etsy Open API | Etsy approval plus API credentials | Listing count when returned, up to 100 sampled listings per request, prices, shops, favorites, tags/materials where available | Primary queue uses quota-guarded wake bursts, respects the live per-second limit, and stops at the 20% reserve required for 80% rolling utilization |
