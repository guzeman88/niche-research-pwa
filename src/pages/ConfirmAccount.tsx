...
export default function ConfirmAccount(){
  const [link]=useState(()=>{const p=new URLSearchParams(window.location.search);return {token_hash:p.get('token_hash'),type:p.get('type'),code:p.get('code')}})
  ...
  return <main className="page max-w-lg mx-auto pt-12"><section className="panel p-6 space-y-5">
    ...
    {link.token_hash||link.code?<button className="btn-primary text-surface-950" disabled={busy} onClick={()=>void confirm()}>{busy?'Verifying…':'Continue securely'}</button>:<p role="alert" className="text-sm text-red-200">This link is incomplete. Request a new verification or recovery email.</p>}
