    ...
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input type="text" className="w-full bg-surface-800 border border-surface-500 rounded-xl pl-10 pr-4 py-3 text-[14px] text-surface-50 placeholder:text-surface-300 focus:outline-none focus:border-primary-500/50 font-medium" placeholder="Enter keywords, e.g. cottagecore art, botanical prints" value={keywords} onChange={e => setKeywords(e.target.value)} disabled={phase === 'running'} onKeyDown={e => e.key === 'Enter' && handleSubmit()} />
        </div>
        {phase === 'running'
          ? <button onClick={() => { eventSourceRef.current?.close(); setPhase('idle') }} className="inline-flex items-center gap-1.5 px-4 py-3 bg-accent-red/10 hover:bg-accent-red/15 text-accent-red text-[13px] font-semibold rounded-xl border border-accent-red/20 transition-colors"><Icon name="square" size={14} />Cancel</button>
          : <button onClick={handleSubmit} disabled={!keywords.trim()} className="inline-flex items-center gap-1.5 px-5 py-3 bg-primary-400 hover:bg-primary-500 disabled:opacity-50 text-surface-50 text-[13px] font-semibold rounded-xl transition-colors"><Icon name="play" size={14} />Research</button>
        }
      </div>

      {phase === 'running' && <LogPanel entries={logs} maxHeight="h-72" />}
      {phase === 'error' && (
        <div className="bg-red-500/5 border border-accent-red/20 rounded-2xl p-4 text-[13px] text-accent-red">
      ...
      {report && (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5">
            <div className="bg-surface-700/80 border border-surface-500/60 rounded-2xl p-3.5"><div className="text-[10px] text-surface-300 uppercase font-semibold tracking-wide">Avg Price</div><div className="text-lg font-extrabold text-surface-50 mt-0.5">{fmtPrice(report.avg_price_usd)}</div></div>
            <div className="bg-surface-700/80 border border-surface-500/60 rounded-2xl p-3.5"><div className="text-[10px] text-surface-300 uppercase font-semibold tracking-wide">Sweet Spot</div><div className="text-lg font-extrabold text-surface-50 mt-0.5">{report.price_sweet_spot || '—'}</div></div>
            <div className="bg-surface-700/80 border border-surface-500/60 rounded-2xl p-3.5"><div className="text-[10px] text-surface-300 uppercase font-semibold tracking-wide">Est. Revenue</div><div className="text-lg font-extrabold text-surface-50 mt-0.5">{fmtPrice(report.estimated_market_monthly_revenue_usd)}</div></div>
            <div className="bg-surface-700/80 border border-surface-500/60 rounded-2xl p-3.5"><div className="text-[10px] text-surface-300 uppercase font-semibold tracking-wide">Comp. Quality</div><div className="text-lg font-extrabold text-surface-50 mt-0.5">{report.avg_competition_quality?.toFixed(0) || '—'}/100</div></div>
          </div>
            ...
            <div className="bg-surface-700/80 border border-surface-500/60 rounded-2xl p-4">
              ...
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {(report.keyword_signals || []).slice(0, 12).map((s, i) => (
                  <div key={i} className="flex items-center justify-between text-[11px] py-1.5 border-b border-surface-500/30 last:border-0">
