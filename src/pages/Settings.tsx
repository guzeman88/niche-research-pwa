  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Configuration and adapter status</p>
      </div>

      {/* Adapter status */}
      <div className="card">
        <h3 className="text-sm font-semibold text-slate-300 mb-3">Adapter Status</h3>
        <div className="space-y-2">
          {adapters && Object.entries(adapters).map(([name, status]) => (
            <div key={name} className="flex items-center justify-between py-2 border-b border-surface-700 last:border-0">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs ${status.healthy ? 'text-emerald-400' : status.available ? 'text-amber-400' : 'text-red-400'}`}>
      ...
      {/* Scoring weights */}
      <div className="card">
        <h3 className="text-sm font-semibold text-slate-300 mb-3">Scoring Configuration</h3>
        {settings?.settings?.niche_scoring ? (
          ...
          <p className="text-sm text-slate-500">Load settings.yaml to configure scoring weights</p>
        )}
      </div>
      ...
      <div className="card">
        ...
        <div className="flex flex-wrap gap-3">
          <a href={getExportCsvUrl()} className="btn-secondary" download>📥 Export CSV</a>
          <a href={getExportJsonUrl()} className="btn-secondary" download>📥 Export JSON</a>
          <a href={getExportJsonUrl(true)} className="btn-secondary" download>📥 Export JSON (with scans)</a>
        </div>
      </div>

      {/* Backend info */}
      <div className="card">
        <h3 className="text-sm font-semibold text-slate-300 mb-2">Backend</h3>
        <p className="text-xs text-slate-500">
          The API server must be running to use the research features.
          Start it with: <code className="text-primary-300 bg-surface-700 px-1.5 py-0.5 rounded">cd backend && uvicorn main:app --reload</code>
        </p>
      </div>
    </div>
  )
